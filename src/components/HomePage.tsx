...
const dummyProducts = [
  { id:1, emoji:'🦾', model_name:'FANUC M-20iB', detail_category:'용접·조립', sale_price:28000000, condition:'2019년식', status:'판매중', bg:'#f0fdf9' },
  { id:2, emoji:'🤖', model_name:'Boston HD6', detail_category:'물류 이송 AGV', sale_price:12500000, condition:'2022년식', status:'판매중', bg:'#f0fdf9' },
  { id:3, emoji:'⚙️', model_name:'ABB IRB 4600', detail_category:'도장·코팅', sale_price:41000000, condition:'2018년식', status:'판매중', bg:'#f0fdf9' },
]

const dummyLifeProducts = [
  { id:4, emoji:'🧹', model_name:'삼성 Jet Bot AI+', detail_category:'로봇청소기', sale_price:650000, condition:'2023년식', bg:'#f8fafc' },
  { id:5, emoji:'🍳', model_name:'Moley Kitchen', detail_category:'조리 로봇', sale_price:3200000, condition:'2021년식', bg:'#f8fafc' },
]

const categories = [
  { id:'mfg', label:'제조업', bg:'#0f172a', icon:(<svg width="22" height="22" viewBox="0 0 22 22" fill="none"><rect x="3" y="8" width="18" height="12" rx="2" stroke="#00d4a1" strokeWidth="1.5"/><path d="M8 8V6a4 4 0 0 1 8 0v2" stroke="#00d4a1" strokeWidth="1.5"/><circle cx="12" cy="14" r="2" fill="#00d4a1"/></svg>)},
  { id:'svc', label:'서비스업', bg:'#0f172a', icon:(<svg width="22" height="22" viewBox="0 0 22 22" fill="none"><circle cx="12" cy="8" r="3" stroke="#00d4a1" strokeWidth="1.5"/><path d="M6 20v-2a6 6 0 0 1 12 0v2" stroke="#00d4a1" strokeWidth="1.5"/></svg>)},
  { id:'home', label:'가정용', bg:'#f1f5f9', icon:(<svg width="22" height="22" viewBox="0 0 22 22" fill="none"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z" stroke="#64748b" strokeWidth="1.5"/></svg>)},
  { id:'clean', label:'청소·관리', bg:'#f1f5f9', icon:(<svg width="22" height="22" viewBox="0 0 22 22" fill="none"><circle cx="12" cy="12" r="9" stroke="#64748b" strokeWidth="1.5"/><path d="M12 7v5l3 3" stroke="#64748b" strokeWidth="1.5" strokeLinecap="round"/></svg>)},
  { id:'wear', label:'웨어러블', bg:'#f1f5f9', icon:(<svg width="22" height="22" viewBox="0 0 22 22" fill="none"><path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5" stroke="#64748b" strokeWidth="1.5" strokeLinecap="round"/></svg>)},
]
...
export default function HomePage({ onNavigate, cartCount, onSelectProduct }: Props) {
  ...
  const displayProducts = dbProducts.length > 0 ? dbProducts : dummyProducts

  const ProductCard = ({ p }: { p: any }) => (
    <div
      onClick={()=>onSelectProduct(p)}
      style={{minWidth:148,background:'#fff',borderRadius:14,border:'0.5px solid #e2e8f0',overflow:'hidden',flexShrink:0,cursor:'pointer'}}>
      ...
      <div style={{padding:'8px 10px 10px'}}>
        <div style={{fontSize:12,fontWeight:500,color:'#0f172a',marginBottom:2}}>{p.model_name}</div>
        <div style={{fontSize:11,color:'#64748b',marginBottom:4}}>{p.detail_category}</div>
        <div style={{fontSize:13,fontWeight:500,color:'#0f172a'}}>₩{p.sale_price?.toLocaleString()}</div>
        <div style={{display:'flex',gap:4,marginTop:4,flexWrap:'wrap'}}>
          <span style={{fontSize:10,padding:'2px 6px',borderRadius:5,fontWeight:500,background:'#fef3c7',color:'#92400e'}}>{p.condition}</span>
    ...
    <div>
      ...
      <div style={{background:'#f8fafc',paddingTop:14}}>
        <div style={{margin:'0 16px',background:'#0f172a',borderRadius:16,padding:'18px 16px',position:'relative',overflow:'hidden'}}>
          ...
          <div style={{fontSize:12,color:'#64748b'}}>이번 주 신규 등록 {dbProducts.length > 0 ? dbProducts.length : 230}건</div>
        ...
        <div style={{display:'flex',gap:10,padding:'0 16px',overflowX:'auto',marginBottom:4,paddingBottom:4}}>
          {categories.map(cat=>(
            <div key={cat.id} style={{minWidth:64,textAlign:'center',cursor:'pointer'}} onClick={()=>onNavigate('cat')}>
              <div style={{width:50,height:50,borderRadius:14,background:cat.bg,display:'flex',alignItems:'center',justifyContent:'center',margin:'0 auto 6px'}}>{cat.icon}</div>
        ...
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'0 16px',marginTop:16,marginBottom:10}}>
          ...
          <span style={{fontSize:11,color:'#94a3b8'}}>실시간</span>
        </div>
        <div style={{display:'flex',gap:10,padding:'0 16px',overflowX:'auto',paddingBottom:4}}>
          {displayProducts.map((p,i)=><ProductCard key={p.id||i} p={p}/>)}
        </div>
        ...
        <div style={{display:'flex',gap:10,padding:'0 16px',overflowX:'auto',paddingBottom:16}}>
          {dummyLifeProducts.map((p,i)=><ProductCard key={p.id||i} p={p}/>)}
        </div>
      </div>
    </div>
  )
